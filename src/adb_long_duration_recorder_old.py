...
class AdbLongDurationRecorder:
    ...
    def adb_echo_string(self, text_to_echo):
        device = self._get_adb_commands_object()
        result = device.Shell('echo {}'.format(text_to_echo))
        return result
    ...
    def adb_start_record(self, duration=-1):
        ...
        try:
            device = self._get_adb_commands_object()
            result = device.Shell('screenrecord  --time-limit {} /sdcard/screen.mp4'.format(duration))
            ...
            ErrorText.ERROR_STARTING_RECORDING
            raise e
